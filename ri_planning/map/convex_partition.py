...
from ..utilities import to_geoms_list, Vector
...
def cut(line: LineString, distance: float) -> Tuple[bool, LineString] :
...
def v_type(p: Vector, vertex: Vector, n: Vector) -> int:
...
class APolygon:
    ...
    dist_tol: float
    delta: int
    cache: Dict[Tuple[Vector, Vector, Vector], LineString]
    nr_vertices: Set[Vector]
    new_diagonals: Set[Tuple[Vector, Vector]]

    def visible(self, vertex: Vector, point: Vector) -> bool:
        line = LineString([vertex, point])
        return self.source.covers(line) and self.source.boundary.touches(line)

    def add_to_cache(self, p: Vector, a: Vector, b: Vector) -> None:
    ...
    def split(self, diagonal: LineString) -> Tuple['APolygon', 'APolygon']:
        try:
            left, right = to_geoms_list(split(self.source, diagonal))
        except ValueError:
            diagonal1 = snap(scale(diagonal, 1.01, 1.01, 1.01,
                                   origin=diagonal.coords[0]), self.source, tolerance=0.01)
            try:
                left, right = to_geoms_list(split(self.source, diagonal1))
            except ValueError as e:
    ...
    def partition(self) -> List['APolygon']:
        ...
        if not diagonal or not diagonal.is_valid or not diagonal.geom_type == 'LineString':
            return [self]
        d = (diagonal.coords[0], diagonal.coords[-1])
        self.new_diagonals.add(d)
        self.new_diagonals.add(d[::-1])
        left, right = to_geoms_list(self.split(diagonal))
