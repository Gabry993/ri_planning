...
from .convex_partition import partition
from .map import Boundary, Cell, Layer, Map, State, Transition
from ..utilities import to_geoms_list


def intersection(g1: BaseGeometry,
                 g2: BaseGeometry,
                 tolerance: float = 0.1) -> BaseGeometry:
    return snap(g2, g1, tolerance=0.1).intersection(g1)


def obstacle(layer: Layer,
             clearance: float,
             tolerance: float = 0.1) -> BaseGeometry:
    ...
    for cell_id, cell in layer.cells.items():
        if not cell.type == 'CellSpace':
            continue
        obstacles.append(
            cell.geometry.buffer(clearance,
                                 resolution=16,
                                 cap_style=3,
                                 join_style=2,
                                 mitre_limit=2.0))
    return unary_union(obstacles).simplify(tolerance)


def all_lines(layers: List[Layer]) -> BaseGeometry:
    return unary_union(
        [b.geometry for layer in layers for b in layer.boundaries.values()])
...
def split_polygon(polygon: Polygon, line: LineString) -> List[Polygon]:
    ...
    return sum([g(x) for x in split(polygon, line).geoms], [])
...
def split_geometry(source: Polygon, lines: BaseGeometry,
    ...
    if lines.geom_type == 'LineString':
        lines = [lines]
    elif lines.geom_type in ('GeometryCollection', "MultiLineString"):
        lines = lines.geoms
    geo = to_geoms_list(geo)
    for line in lines:
        geo = sum((split_polygon(p, line) for p in geo), [])
        # geo = sum((list(split(p, l)) for p in geo), [])
    return geo


def f(g1: LineString, g2: LineString) -> bool:
    return cast(bool, g1.relate_pattern(g2, 'T********'))
...
def add_copy_of_cell(layer: Layer, cell: Cell,
                     geometry: List[Polygon]) -> List[Cell]:
    cells = []
    ...
    for cell1, cell2 in combinations(cells, 2):
        if f(cell1.geometry.boundary, cell2.geometry.boundary):
            ...
            line = intersection(cell1.geometry.boundary,
                                cell2.geometry.boundary)
            ls: List[LineString] = []
...
def add_boundary(layer: Layer, cell: Cell, cell2: Optional[Cell],
    ...
    for c in [cell, cell2]:
        ...
        for pb in list(c.boundary):
            if f(pb.geometry, line):
                ...
                if pline.geom_type == 'MultiLineString':
                    pb.geometry = pline.geoms[0]
                    if len(pline.geoms) > 1:
                        c1, c2 = pb.cells
                        rest_boundaries.append((c1, c2, pline.geoms[1]))
                    pline = pline.geoms[0]
                else:
                    pb.geometry = pline
                if pb.duality:
    ...
    if cell2:
        ...
        layer.graph.add_edge(t.start.id,
                             t.end.id,
                             t.id,
                             id=t.id,
                             weight=t.weight)
...
def from_layer(source: Layer,
               uid: str,
               clearance: float,
               tolerance: float = 0.1,
               intersect: List[Layer] = [],
               convex: bool = True) -> Layer:
    layer = Layer(uid)
    layer.description = (
        f'Layer obtained from {source} adding a clearance of {clearance} cm')
    ...
    layers = {layer.id: layer for layer in intersect}
    for cell_id, cell in tqdm(source.cells.items()):
        ...
        boundaries = set(
            sum([
                list(layers[k].states[x].duality.boundary)
                for k, xs in state.contains.items() for x in xs if k in layers
            ], []))
        lines = unary_union([b.geometry for b in boundaries])

        geometry = split_geometry(cell.geometry, lines, obs)
        if convex:
            geometry = sum([
                partition(p, dist_tol=1, abs_tol=10, rel_tol=0.001)
                for p in geometry
            ], [])
...
def navigation_layer(map_: Map,
                     uid: str,
                     clearance: float,
                     tolerance: float = 0.1,
                     convex: bool = True) -> Layer:

    geometric_layer = next(
        layer for layer in map_.layers.values()
        if layer.cls_name == 'TOPOGRAPHIC' and layer.usage == 'Geometry')
    layers = [
        layer for layer in map_.layers.values()
        if layer.cls_name != 'TOPOGRAPHIC'
    ]
    return from_layer(geometric_layer,
                      uid=uid,
                      clearance=clearance,
                      tolerance=tolerance,
                      intersect=layers,
