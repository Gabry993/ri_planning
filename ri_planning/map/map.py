...
from typing import (Any, Callable, Collection, Dict, Iterator, List, Mapping,
...
def meta2dict_partial(
        meta: ET.Element) -> Dict[Tuple[Optional[str], str], Any]:
...
_ns2types = {
    'indoorCore': [
        'CellSpace', 'CellSpaceBoundary', 'SpaceLayer', 'MultiLayeredGraph',
        'State', 'Transition'
    ],
    'indoorNavi': [
        'NavigableSpace', 'NavigableBoundary', 'GeneralSpace', 'TransferSpace',
...
def gml_id(node: ET.Element) -> str:
    return cast(str, node.get('{%s}id' % nsmap['gml']))


def gml_pos(gml_pos: Optional[ET.Element],
            precision: int = 1) -> Optional[Vector]:
    if gml_pos is not None:
        if gml_pos.text is not None:
            coords: List[float] = [
                round(float(n), precision) for n in gml_pos.text.split()
            ]
            return tuple(coords[:2])  # type: ignore
    return None


def gml_pos_list(gml_pos_list: Optional[ET.Element],
                 precision: int = 1) -> Optional[List[Vector]]:
    if gml_pos_list is not None:
        if gml_pos_list.text:
            coord = [
                round(float(n), precision) for n in gml_pos_list.text.split()
            ]
            return list(zip(coord[::2], coord[1::2]))
    return None


def polygon_for_bb(bb: BoundingBox) -> Polygon:
    minx, miny, maxx, maxy = bb
    return Polygon(((minx, miny), (maxx, miny), (maxx, maxy), (minx, maxy)))


def state_has_stair(state: 'State') -> bool:
    if state.duality and state.duality.type == 'TranferSpace' and state.duality.function:
        if 'Stair' in state.duality.function or any(
                state_has_stair(s) for s in state.up):
            return True
    return False


def reverse(edge: GraphEdge) -> GraphEdge:
    return edge[1], edge[0], edge[2]


def reverse_route(route: List[GraphEdge]) -> List[GraphEdge]:
    return [reverse(edge) for edge in route[::-1]]
...
class GMLFeature(ABC):
    ...
    def _repr_svg_(self) -> str:
        from .display.svg import svg_for
        return svg_for(self)
    ...
    def xml_root(self) -> ET.Element:
        ...
        root.set(name_ns('schemaLocation', 'xsi'),
                 ' '.join(["%s %s" % i for i in schemas.items()]))
        return root
    ...
    def bounds(self) -> Optional[BoundingBox]:
        if self.geometry:
            return cast(BoundingBox, self.geometry.bounds)
        return None
...
class State(GMLFeature):
    ...
    def has_sensor(self) -> bool:
        return (self.layer.cls_name == 'SENSOR' and self.name is not None
                and self.name != 'NO' and self.duality is not None)
    ...
    def xml(self, ref: Optional[Set[str]] = None) -> ET.Element:
        ...
        for transition in self.connects:
            ET.SubElement(root, core('connects'), {xlink: f"#{transition.id}"})
        if self.geometry:
    ...
    def distance(self, position: Vector) -> float:
        ...
        if p.intersects(polygon):
            return 0
        return cast(float, p.distance(polygon.exterior))

    def nearest_point(self, position: Vector) -> Optional[Vector]:
        """ returns:: (distance,  nearest position in state) """
        p = Point(position)
        if not self.duality:
            return None
        ...
        return cast(Vector, n_point.coords[0])

    def in_layer(self, layer: 'Layer') -> Optional['State']:
        if layer == self.layer:
            return self
        candidates = [layer.states[s] for s in self.inside[layer.id]]
        if candidates:
            return candidates[0]
        return None
...
class Transition(GMLFeature):
    ...
    def init_from_xml(self, node: ET.Element, layer: 'Layer') -> None:
        ...
        line: Optional[List[Vector]] = None
    ...
    def xml(self, ref: Optional[Set[str]] = None) -> ET.Element:
        ...
        for state in self.connects:
            ET.SubElement(root, core('connects'), {xlink: f"#{state.id}"})
        if self.duality:
            ET.SubElement(root, core('duality'),
                          {xlink: f"#{self.duality.id}"})
        if self.geometry:
...
class Cell(GMLFeature):
    ...
    def init_from_xml(self, node: ET.Element, layer: 'Layer') -> None:
        ...
        self.boundary = set()
        for boundary_node in node.findall('indoorCore:partialboundedBy',
                                          nsmap):
        ...
        if polygonXML is None:
            ...
            for pos_list in polygonXML.findall('gml:exterior//gml:posList',
                                               nsmap):
                exterior = gml_pos_list(pos_list)

            for loop in polygonXML.findall('gml:interior//gml:LinearRing',
                                           nsmap):
                ls = None
...
class Boundary(GMLFeature):
    ...
    def init_from_xml(self, node: ET.Element, layer: 'Layer') -> None:
        line: Optional[List[Vector]] = None
        for pos_list in node.findall(
                'indoorCore:geometry2D/gml:LineString/gml:posList', nsmap):
            line = gml_pos_list(pos_list)
        if line:
            self.geometry = LineString(line)
        if not self.geometry.is_valid:
...
class Layer(GMLFeature):
    ...
    def from_xml(cls, node: ET.Element) -> 'Layer':  # type: ignore
        ...
        for n in node.findall("indoorCore:nodes//indoorCore:partialboundedBy",
                              nsmap):
        ...
        for n in node.findall("indoorCore:edges//indoorCore:Transition",
                              nsmap):
        ...
        for i, transition in layer.transitions.items():
            ...
            layer.graph.add_edge(transition.start.id,
                                 transition.end.id,
                                 i,
                                 id=i,
    ...
    def nearest_states(
            self, position: Vector) -> Iterator[Tuple[float, State, Point]]:
        heap: List[Tuple[float, State, Point]] = []
        p = Point(position)
        for i in self.rtree_index.nearest(position,
                                          num_results=len(self.states),
                                          objects=True):
            s = self.states[i.object]
            d = s.distance(position)
            cell = cast(Cell, s.duality)
            max_d = polygon_for_bb(cell.geometry.bounds).distance(p)
            np = s.nearest_point(position)
            heappush(heap, (d, s, np))
            while True:
                d, s, np = heappop(heap)
                if d < max_d:
                    yield (d, s, np)
    ...
    def state_with_position(self, position: Vector) -> Optional[State]:
        p = Point(position)

        def disjoint(state: State) -> bool:
            cell = state.duality
            if cell:
                return cast(bool, cell.geometry.disjoint(p))
            return True

        states = [
            i.object
            for i in self.rtree_index.intersection(position, objects=True)
            if not disjoint(self.states[i.object])
        ]
        if len(states):
    ...
    def nearest_state_among(
        self,
        position: Vector,
        states_with: Callable[[State],
                              bool] = lambda s: True) -> Optional[State]:

        for distance, state, _ in self.nearest_states(position):
            if states_with(state):
                return state
        return None

    def state_at(self, position: Vector) -> Optional[State]:
        p = Point(position)
        try:
            i = next(
                i.object
                for i in self.rtree_index.intersection(position, objects=True)
                if not cast(Cell, self.states[
                    i.object].duality).geometry.disjoint(p))
            return self.states[i]
        except StopIteration:
            return None

    def states_crossing_box(self, bounding_box: BoundingBox) -> List[State]:
        polygon = polygon_for_bb(bounding_box)
        return [
            self.states[i.object]
            for i in self.rtree_index.intersection(bounding_box, objects=True)
            if polygon.intersects(
                cast(Cell, self.states[i.object].duality).geometry)
        ]
    ...
    def geometry(self) -> BaseGeometry:  # type: ignore
        return unary_union([cell.geometry for cell in self.cells.values()])
...
class Map(GMLFeature, MappingABC):
    ...
    @property
    def geometry(self) -> BaseGeometry:  # type: ignore
        return unary_union([layer.geometry for layer in self.layers.values()])
    ...
    def init_interlayer_edges(self) -> None:
        ...
        relations: List[Tuple[de9im.AnyPattern, str, str]] = [
            (de9im.equal, 'equals', 'equals'),
            (de9im.within, 'inside', 'contains'),
            (de9im.contains, 'contains', 'inside'),
            (de9im.overlaps_regions, 'overlaps', 'overlaps'),
            (de9im.intersects, 'intersects', 'intersects'),
            (de9im.touches, 'touches', 'touches')
        ]
        ...
        def area(state_id: str) -> float:
            cell = self.states[state_id].duality
            if cell:
                return cast(float, cell.geometry.area)
            return 0.0

        for i, s in self.states.items():
            empty: Set[str] = set()
            ws: Collection[Set[str]] = s.inside.values() or [empty]
            s.up = [
                self.states[x]
                for x in sorted(set.union(*ws), key=area, reverse=False)
            ]
            cs: Collection[Set[str]] = s.contains.values() or [empty]
            s.down = [
                self.states[x]
                for x in sorted(set.union(*cs), key=area, reverse=True)
            ]

    def xml(self, ref: Optional[Set[str]] = None) -> ET.Element:
        root = super(Map, self).xml()
        layers_node = ET.SubElement(root, core('spaceLayers'),
                                    {gml('id'): "layers"})
